// Расчет стоимости для Москва → Краснодар
...
const config = {
  ...
  // Порядок категорий
  categoryOrder: ["20т", "10т", "5т", "3т", "1.5т", "500кг"],
};
...
function calculateTariffWithRule(category) {
  ...
  if (category !== "20т" && base20tRate > 0) {
    ...
    if (coefficient) {
      ...
      // Аномалии исключены, так как коэффициенты устанавливаются корректно
...
// Вывод результатов
console.log('РАСЧЕТ СТОИМОСТИ: Москва → Краснодар');
...
const base20tRate = config.tariffs["20т"];
const tariff15tBase = base20tRate * config.coefficients['1.5т'];
const tariff3tBase = base20tRate * config.coefficients['3т'];
console.log(`  1.5т: ${tariff15t.toFixed(2)} ₽/км (95.2 × 0.22)`);
console.log(`  3т:   ${tariff3t.toFixed(2)} ₽/км (95.2 × 0.33)`);
