...
function getTariffForCategory(route, category) {
  switch(category) {
    ...
    case "10т": return route.tariff10t > 0 ? route.tariff10t : route.tariff20t * 0.81;
...
// Сначала рассчитаем старую логику для 6 м³, чтобы получить целевое значение
const category6m3 = getTruckCategoryForVolume(6);
const truckCapacity6m3 = TRUCK_CAPACITY[category6m3];
const tariff6m3 = getTariffForCategory(route, category6m3);
const fullTruckCost6m3 = tariff6m3 * route.distance;
const loadFactor6m3 = Math.max(6 / truckCapacity6m3, MIN_LOAD_FACTOR);
const TARGET_COST_6M3 = Math.max(Math.round(fullTruckCost6m3 * loadFactor6m3), MINIMUM_COST);

// Подбираем стоимость 1 м³ для каждой категории объемов
// Для 1-6 м³: подбираем так, чтобы 6 м³ = целевая стоимость из старой логики
const costPerM3_1_6 = (TARGET_COST_6M3 - PODACHA) / 6;

// Для 7-12 м³: используем стоимость от 20т, но подбираем podacha так, чтобы 7 м³ >= 6 м³
const costPerM3_7_12 = full20tCost / TRUCK_20T_CAPACITY; // стоимость от 20т фуры
const cost6m3 = PODACHA + (6 * costPerM3_1_6); // должна быть = TARGET_COST_6M3
...
// Получаем стоимость 13 м³ из старой логики для сравнения
const category13m3 = getTruckCategoryForVolume(13);
const truckCapacity13m3 = TRUCK_CAPACITY[category13m3];
const tariff13m3 = getTariffForCategory(route, category13m3);
const fullTruckCost13m3 = tariff13m3 * route.distance;
const loadFactor13m3 = Math.max(13 / truckCapacity13m3, MIN_LOAD_FACTOR);
const oldCost13m3 = Math.max(Math.round(fullTruckCost13m3 * loadFactor13m3), MINIMUM_COST);
const target13m3 = Math.max(cost12m3, oldCost13m3) + 200;
const costPerM3_13_20 = full20tCost / TRUCK_20T_CAPACITY; // 831.60 ₽
const PODACHA_13_20 = target13m3 - (13 * costPerM3_13_20);

console.log('МОСКВА → РОСТОВ-НА-ДОНУ');
...
console.log(`Целевая стоимость 6 м³ (старая логика): ${TARGET_COST_6M3} ₽`);
